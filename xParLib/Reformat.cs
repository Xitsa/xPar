    ...
    public static class ReformatModule
    {
            ...
            bool touch)
        {
            ...
            while (numout < hang || wordIdx2.HasValue)
            {
                ...
                if (wordIdx2.HasValue)
                {
                    // Здесь по  сути реализован аналог  алгоритма Брезенхема:
                    // распределить extra  пробелов по numgaps  позициям между
                    // слов.
                    //
                    // Соответственно, движение по  оси numgaps сопровождается
                    // вычислением   избытка  по   оси  extra   с  добавлением
                    // пробелов.
                    //
                    // Начальная установка phase  в numgaps / 2 необходима для
                    // смещения распределения  пробелов в центр, чтобы  они не
                    // копились преимущественно в начале или конце строк.
                    int phase = numgaps / 2;
